...
class ChromHAIconView(HomeAssistantView):
    ...
    url = ICON_AUTO_URL_BASE + "/{tier}/{condition}.svg"
    ...
    async def get(
        self, request: web.Request, tier: str, condition: str
    ) -> web.Response:
        # Allowlist by condition name, so no part of the path comes from the
        # request unchecked.
        if condition not in WEATHER_ICON_MAP or tier not in ("1", "2", "3"):
            return web.Response(status=404)

        night = self._is_night()
        filename = icon_map(night, int(tier))[condition]
