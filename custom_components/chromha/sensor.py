...
class ChromHAPaletteSensor(ChromHAEntity, SensorEntity):
    ...
    def extra_state_attributes(self) -> dict:
        ...
        tier = INTENSITY_TIERS.get(
            options.get(CONF_ICON_INTENSITY, DEFAULTS[CONF_ICON_INTENSITY]), 2
        )
        attrs["weather_icons_are_night"] = use_night
        attrs["icon_intensity"] = options.get(
            CONF_ICON_INTENSITY, DEFAULTS[CONF_ICON_INTENSITY]
        )
        ...
        if options.get(CONF_ICON_DAYNIGHT, DEFAULTS[CONF_ICON_DAYNIGHT]) == DAYNIGHT_SUN:
            ...
            attrs["weather_icons"] = {
                condition: f"{ICON_AUTO_URL_BASE}/{tier}/{condition}.svg"
                for condition in WEATHER_ICON_MAP
            }
        else:
            attrs["weather_icons"] = {
                condition: f"{ICON_URL_BASE}/{filename}.svg"
                for condition, filename in icon_map(use_night, tier).items()
            }
        return attrs
