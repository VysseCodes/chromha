...
CONF_ICON_INTENSITY: Final = "icon_intensity"
...
# Intensity tiers for rain and snow.
INTENSITY_LIGHT: Final = "Light"
INTENSITY_STANDARD: Final = "Standard"
INTENSITY_HEAVY: Final = "Heavy"
INTENSITIES: Final = [INTENSITY_LIGHT, INTENSITY_STANDARD, INTENSITY_HEAVY]
INTENSITY_TIERS: Final[dict[str, int]] = {
    INTENSITY_LIGHT: 1,
    INTENSITY_STANDARD: 2,
    INTENSITY_HEAVY: 3,
}
...
DEFAULTS: Final[dict] = {
    ...
    CONF_ICON_INTENSITY: INTENSITY_STANDARD,
    CONF_CONTRAST: False,
}
...
#   * intensity tiers for rain, snow and cloud
...
#     tropical storm)
#
# `{tier}` is substituted with 1, 2 or 3 and `{dn}` with -day or -night.
...
def icon_map(night: bool = False, tier: int = 2) -> dict[str, str]:
    """Condition to bundled filename, for a given sun state and intensity."""
    ...
    for condition, template in _ICON_TEMPLATES.items():
        ...
        if template in _NO_DAYNIGHT or template == "clear-night":
            out[condition] = template.replace("{tier}", str(tier))
            continue
        dn = "-night" if night else "-day"
        out[condition] = template.format(tier=tier, dn=dn)
    return out
